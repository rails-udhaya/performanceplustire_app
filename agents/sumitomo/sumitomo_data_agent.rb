																								...
																								Headless.ly do		
																													 ...
																													 browser.text_field(:id, 'ctl00_Login_Password').set($site_details["sumitomo_password"])
																														...
																														temp_1 = doc.css("table#tableall tbody tr.promotedItem2")
																																temp_1.each do |t_1|
																																		...
																																		seller_cost = ""
																																		dc_quantity = ""
																																		puts quantity= t_1.previous_element.css("td")[1].text.strip() if !t_1.previous_element.css("td")[1].nil? && t_1.css("td")[1]
																																puts inventory_number = "A+1:#{t_1.css("td")[2].text.strip()}:D" if !t_1.css("td")[2].nil? && t_1.css("td")[2] 		
																														...
																														temp_2 = doc.css("table#tableall tbody tr.oddRow2")
																																temp_2.each do |t_2|
																																		...
																																		seller_cost = ""
																																		dc_quantity = ""
																																		puts quantity= t_2.previous_element.css("td")[1].text.strip() if !t_2.previous_element.css("td")[1].nil? && t_2.css("td")[1]
																																puts inventory_number = "A+1:#{t_2.css("td")[2].text.strip()}:D"		 if !t_2.css("td")[2].nil? && t_2.css("td")[2]
																																dc_quantity = "Long Beach=#{quantity}".strip() if quantity
																																		SumitomoData.create(:inventory_number=>inventory_number, :quantity=>quantity, :quantity_updated_type=>quantity_updated_type, :seller_cost => seller_cost, :dc_quantity => dc_quantity)
																														end			
																										...
																										temp_3 = doc.css("table#tableall tbody tr.evenRow2")
																																temp_3.each do |t_3|
																																		...
																																		seller_cost = ""
																																		dc_quantity = ""
																																		puts quantity= t_3.previous_element.css("td")[1].text.strip() if !t_3.previous_element.css("td")[1].nil? && t_3.css("td")[1]
																																puts inventory_number = "A+1:#{t_3.css("td")[2].text.strip()}:D"		 if !t_3.css("td")[2].nil? && t_3.css("td")[2]
																																dc_quantity = "Long Beach=#{quantity}".strip() if quantity
																																		SumitomoData.create(:inventory_number=>inventory_number, :quantity=>quantity, :quantity_updated_type=>quantity_updated_type, :seller_cost => seller_cost, :dc_quantity => dc_quantity)
																														end

																														#~ puts temp_file_path="#{Rails.root}/public/sumitomo_preprocessed_data/"
																														#~ puts latest_temp_file_path = Dir.glob(File.join(temp_file_path, '*.*')).max { |a,b| File.ctime(a) <=> File.ctime(b) }
																														#~ xlsx = Roo::Spreadsheet.open(latest_temp_file_path)
																																...
																																#~ if(!row[0].value.nil? && row[0].value != "" &&row[0].value.class.to_s != "Date" && row[0].value.strip() != "Site ID and Name")
																																		#puts row[0].value
																																		#~ puts inventory_number= row[5].value.gsub(/^SUMI-/,"A+1:")+":D" if !row[5].value.nil?
																																		#~ puts quantity= row[6].value.to_i.to_s if !row[6].value.nil?
																																		#~ puts dc_quantity = "Long Beach=" +  row[6].value.to_i.to_s if  row[6].value.to_i
																																		#~ SumitomoData.create(:inventory_number=>inventory_number, :quantity=>quantity, :quantity_updated_type=>quantity_updated_type, :seller_cost => seller_cost, :dc_quantity => dc_quantity)
																																#~ end
																														#~ end
																														browser.close
